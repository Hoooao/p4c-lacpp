...
class CodeGraphJSONDataset(Dataset):
    ...
    """
    def __init__(self, folder_path, dataset_number=0):
        super().__init__()
    ...
    def _load_samples(self):
        ...
        for folder in folders[:self.dataset_number+1]:
            for file in folder.rglob("*.json"):
                ...
                samples.append(Data(x=x,
                                    ...
                                    edge_attr=edge_attr,
                                    y=y))
        print(f"Loaded {len(samples)} samples from {self.dataset_number+1} folders.")
        return samples
...
def train_model(model, dataloader, batch_size=1):
    ...
    for epoch in range(EPOCHS):
        ...
        avg_loss = round(total_loss / len(dataloader), 5)
...
if __name__ == "__main__":
    ...
    for batch_size in BATCH_SIZES:
        ...
        # save loss history
        with open(f"loss_history_b{batch_size}.json", "w") as f:
