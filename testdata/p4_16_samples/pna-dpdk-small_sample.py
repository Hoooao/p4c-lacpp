...
class Test1(AbstractTest):
    # Date generated: 2023-08-28-12:04:22.720
    '''
    # Current statement coverage: nan
    [Parser] MainParserImpl
    [State] start
    ...
    [Control MainControlImpl start]
    [Internal If Statement]: 1; -> 1; -> true
    [Table Branch: MainControlImpl.ipv4_da Choosing default action: next_hop;]
    [send_to_port executed]
    [Control MainDeparserImpl start]
    [Emit] hdr.ethernet -> hdr.ethernet.dstAddr = 0x0000_0000_0000 | hdr.ethernet.srcAddr = 0x0000_0000_0000 | hdr.ethernet.etherType = 0x0800
    [Emit] hdr.ipv4 -> hdr.ipv4.version = 0x0 | hdr.ipv4.ihl = 0x0 | hdr.ipv4.diffserv = 0x00 | hdr.ipv4.totalLen = 0x0000 | hdr.ipv4.identification = 0x0000 | hdr.ipv4.flags = 0x0 | hdr.ipv4.fragOffset = 0x0000 | hdr.ipv4.ttl = 0x00 | hdr.ipv4.protocol = 0x00 | hdr.ipv4.hdrChecksum = 0x0000 | hdr.ipv4.srcAddr = 0x0000_0000 | hdr.ipv4.dstAddr = 0x0000_0000
...
class Test2(AbstractTest):
    # Date generated: 2023-08-28-12:04:22.726
    '''
    # Current statement coverage: nan
    [Parser] MainParserImpl
    [State] start
    ...
    def sendPacket(self):
        ig_port = 0
        pkt = b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x08\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
        ptfutils.send_packet(self, ig_port, pkt)

    def verifyPackets(self):
        eg_port = 0
        exp_pkt = Mask(b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x08\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00')
        ptfutils.verify_packet(self, exp_pkt, eg_port)
        bt.testutils.log.info("Verifying no other packets ...")
        ptfutils.verify_no_other_packets(self, self.device_id, timeout=self.packet_wait_time)
...
class Test3(AbstractTest):
    # Date generated: 2023-08-28-12:04:22.733
    '''
    # Current statement coverage: nan
    [Parser] MainParserImpl
    [State] start
    ...
    [Internal If Statement]: 0; -> 0; -> false
    [Control MainDeparserImpl start]
    [Emit] hdr.ethernet -> hdr.ethernet.dstAddr = 0x0000_0000_0000 | hdr.ethernet.srcAddr = 0x0000_0000_0000 | hdr.ethernet.etherType = 0x0000
    [Invalid emit: hdr.ipv4]
    [Prepending the emit buffer to the program packet]
    [Internal If Statement]: 0; -> 0; -> false
    '''
    ...
    def sendPacket(self):
        ig_port = 0
        pkt = b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
        ptfutils.send_packet(self, ig_port, pkt)

    def verifyPackets(self):
        eg_port = 0
        exp_pkt = Mask(b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00')
        ptfutils.verify_packet(self, exp_pkt, eg_port)
        bt.testutils.log.info("Verifying no other packets ...")
        ptfutils.verify_no_other_packets(self, self.device_id, timeout=self.packet_wait_time)
...
class Test4(AbstractTest):
    # Date generated: 2023-08-28-12:04:22.739
    '''
    # Current statement coverage: nan
    [Parser] MainParserImpl
    [State] start
    [ExtractFailure] hdr.ethernet@0 | Condition: !(|*packetLen_bits(bit<32>)| >= 112); | Extract Size: 112
    [Control PreControlImpl start]
    [Control MainControlImpl start]
    [Internal If Statement]: 0; -> 0; -> false
    [Control MainDeparserImpl start]
    [Invalid emit: hdr.ethernet]
    [Invalid emit: hdr.ipv4]
    [Prepending the emit buffer to the program packet]
    [Internal If Statement]: 0; -> 0; -> false
    '''
